document.addEventListener('alpine:init', () => {
	...
	Alpine.data('quickAddForm', () => ({
		...
		init() {
			...
			const { product, variant_to_use, variant_images, options_with_values } =
				data;
			...
			setTimeout(() => {
				...
				this.$el.style.setProperty(
					'height',
					`${Math.min($innerContent.scrollHeight + 48, 460)}px`,
					'important',
				);
			}, 500);
			...
			this.$watch('options', () => {
				const sortedOptions = Object.values(this.options)
				...
				if (this.featuredImageSrc != newFeaturedImage) {
					...
					if (this.$image && this.$image instanceof HTMLImageElement) {
						if (this.featuredImageSrc) this.$image.src = this.featuredImageSrc;
					}
				}
							...
							if (variant.options[idx] === val) {
								...
								product.options.forEach((otherOptName, otherIdx) => {
									if (otherIdx !== idx) {
										...
										if (
											!this.compatible[optionName][val].includes(otherValue)
										) {
